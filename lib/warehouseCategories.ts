...
export function warehousePayloadToDataset(
  category: Category,
  payload: WarehousePayload,
): CategoryDataset {
  if (payload.computedPlayableV15 !== true) {
    throw new Error(
      `${category.shortName} is not approved by the authoritative v15 catalog.`,
    );
  }
  ...
  if (observations.length < category.coverageFloor) {
    throw new Error(
      `${category.shortName} has only ${observations.length} approved common-year countries; ${category.coverageFloor} are required.`,
    );
  }
  ...
  const enrichedCategory: Category = {
    ...
    playerSourceStatus:
      (payload.playerSourceStatus as Category["playerSourceStatus"]) ??
      category.playerSourceStatus,
    playerSourceReason:
      payload.playerSourceReason ?? category.playerSourceReason,
    playerSourceCheckedAt:
      payload.playerSourceCheckedAt ?? category.playerSourceCheckedAt,
    contentReviewStatus:
      (payload.contentReviewStatus as Category["contentReviewStatus"]) ??
      category.contentReviewStatus,
    contentReviewReason:
      payload.contentReviewReason ?? category.contentReviewReason,
    contentReviewVersion:
      payload.contentReviewVersion ?? category.contentReviewVersion,
    immediateComprehensionScore:
      payload.immediateComprehensionScore ??
      category.immediateComprehensionScore,
    gameplayInterestScore:
      payload.gameplayInterestScore ?? category.gameplayInterestScore,
    ...
    evidenceLabel:
      (payload.evidenceLabel as Category["evidenceLabel"]) ??
      category.evidenceLabel,
    credibilityScore: payload.credibilityScore ?? category.credibilityScore,
    trustStatus:
      (payload.trustStatus as Category["trustStatus"]) ?? category.trustStatus,
    trustReason: payload.trustReason ?? category.trustReason,
    verifiabilityScore:
      payload.verifiabilityScore ?? category.verifiabilityScore,
    verifiabilityStatus:
      payload.verifiabilityStatus ?? category.verifiabilityStatus,
    understandabilityScore:
      payload.understandabilityScore ?? category.understandabilityScore,
    funScore: payload.funScore ?? category.funScore,
    objectiveStatus:
      (payload.objectiveStatus as Category["objectiveStatus"]) ??
      category.objectiveStatus,
    playerQualityStatus:
      (payload.playerQualityStatus as Category["playerQualityStatus"]) ??
  ...
  return {
    ...
    observations,
    year: String(
      payload.commonYear ?? observations[0]?.year ?? "Latest available",
    ),
    ...
    immediateComprehensionScore:
      payload.immediateComprehensionScore ?? undefined,
...
export async function fetchWarehouseCategory(
  category: Category,
  ...
  const payload = (await response
    .json()
    .catch(() => ({}))) as WarehousePayload;

  if (!response.ok) {
    throw new Error(
      payload.error ||
        `${category.shortName} warehouse data could not be loaded.`,
    );
  }
