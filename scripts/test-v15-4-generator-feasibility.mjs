...
const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), "geostats-v15-4-generator-"));
...
try {
  ...
  function makeDataset(index) {
    ...
    return canonicalizeDataset({
      category: {
        ...
        broadDomain: source === "naturalearth" ? "physical-geography" : type.toLowerCase(),
        knowledgeCluster: `knowledge-cluster-${index}`,
  ...
  console.log("GeoStats v15.4 synthetic Daily feasibility stress test passed.");
} finally {
