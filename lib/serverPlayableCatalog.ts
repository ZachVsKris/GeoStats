...
export async function loadServerPlayableCategoryCatalog(): Promise<Category[]> {
  ...
  if (!v14.error && v14.data?.length) return buildPlayableCategoryCatalog(v14.data);
  if (v14.error && !/plain_language_description|source_page_url|verifiability_|player_quality_|objective_status|input_datasets|dataset_release/i.test(v14.error.message)) {
    throw new Error(v14.error.message);
  }
  const modern = await queryRows(V13_SELECT, true);
  if (!modern.error && modern.data?.length) return buildPlayableCategoryCatalog(modern.data);
  if (modern.error && !/credibility_|evidence_label/i.test(modern.error.message)) throw new Error(modern.error.message);
