...
function blockerTone(category: CategoryRow) {
  if (category.computed_playable_v16) return "playable";
...
export default function CategoryReviewWorkbench() {
  ...
  const loadQueue = useCallback(async (preferredId?: string | null) => {
    ...
    setLoadingQueue(false);
  }, [committedQuery, source, status, readiness, audit, sort, page, selectedId]);
  ...
  useEffect(() => {
    void loadQueue();
  }, [committedQuery, source, status, readiness, audit, sort, page]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <article className="reviewDetailPane">
        ...
        {current && <>
          <div className="reviewDetailHeader">
            ...
            <div className={`reviewReadinessBadge ${current.computed_playable_v16 ? "playable" : current.hard_gate_ready ? "ready" : "blocked"}`}>
              {current.computed_playable_v16 ? "Playable" : current.hard_gate_ready ? "Integrity-ready" : "Hard blockers"}
            </div>
          </div>
          ...
          <div className="reviewDetailGrid">
            ...
            <section className="reviewPanel">
              ...
              {current.hard_gate_ready
                ...
                : <div className="reviewBlockerList">{(current.v16_blockers ?? []).filter((blocker) => !blocker.startsWith("Editorial") && !blocker.startsWith("Flagged") && !blocker.startsWith("Marked")).map((blocker) => <div key={blocker}>{blocker}</div>)}</div>}
