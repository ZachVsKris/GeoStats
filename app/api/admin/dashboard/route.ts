...
export async function GET() {
  ...
  const computedCategoryRows = categoryRows.map((row) => ({
    ...row,
    computed_playable: row.computed_playable_v16 === true,
    playability_blockers: row.v16_blockers ?? [],
    playability_warnings: row.v16_warnings ?? [],
  ...
  const [integrityOverviewResult, integrityBySourceResult, integrityIssuesResult, validationRunsResult, semanticConflictsResult, contentLinkOverviewResult, contentLinkIssuesResult] = await Promise.all([
    admin.from("data_integrity_overview_v16").select("*").maybeSingle(),
    admin.from("data_integrity_by_source_v16").select("*").order("source"),
    admin.from("data_integrity_issues_v16").select("*").limit(50),
