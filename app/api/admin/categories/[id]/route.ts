...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const reviewHistory = (reviews.data ?? []).map((row: { next_state?: Record<string, unknown>; created_at: string }) => ({
    decision: typeof row.next_state?.status === "string" ? row.next_state.status : "updated",
    notes: typeof row.next_state?.notes === "string" ? row.next_state.notes : null,
    created_at: row.created_at,
  }));
  return NextResponse.json({ category, year, top: top.data ?? [], bottom: bottom.data ?? [], reviews: reviewHistory });
}
