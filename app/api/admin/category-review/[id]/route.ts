...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!previous) return NextResponse.json({ error: "Category review state not found. Run the v16 SQL installer." }, { status: 409 });
  ...
  const categoryUpdate: Record<string, unknown> = {
    ...
    curation_reason: `GeoStats v16 authoritative category review state: ${policy.editorial_status}.`,
    curation_version: "geostats-v16-review-v1",
    content_review_status: legacyEditorialStatus,
    content_review_reason: `GeoStats v16 authoritative category review state: ${policy.editorial_status}.`,
    content_review_version: "geostats-v16-review-v1",
    player_quality_status: policy.editorial_status === "approved"
      ? "approved"
      : ["rejected", "duplicate"].includes(policy.editorial_status)
        ? "blocked"
        : "caution",
    player_quality_reason: `GeoStats v16 authoritative category review state: ${policy.editorial_status}.`,
