...
wb_crossings = module.observed_threshold_crossing_years(wb_series, 50.0)
...
for spec_row in module.WORLD_BANK_MILESTONE_SPECS:
    ...
    for index, iso3 in enumerate(["USA", "CAN", "FRA", "DEU", "GBR", "JPN", "BRA", "ARG", "MEX", "CHL"]):
        year = 2000 + index
        fixture_rows.extend([
            {"countryiso3code": iso3, "date": str(year - 1), "value": threshold - 1},
            {"countryiso3code": iso3, "date": str(year), "value": threshold + 1},
        ])
...
assert set(wb_candidates) == {"majority-urban", "internet-half", "electricity-half", "life-expectancy-70"}
...
print("GeoStats v16.2.4 historical importer tests passed.")
