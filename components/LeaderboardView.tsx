...
export default function LeaderboardView() {
  ...
  return <section className="leaderboardPage panel">
    ...
    {loading ? <div className="leaderboardEmpty">Loading {config.label} leaderboard…</div> : rows.length ? <div className={`publicLeaderboard ${tab === "alltime" ? "allTimeLeaderboard" : ""}`}>
      ...
      {tab === "today" ? today.map((leader, index) => <div key={`${leader.username}-${index}`}><b>{index + 1}</b><span>{leader.username}</span><strong>{leader.score}</strong><span>{leader.averagePlacement.toFixed(1)}</span><span>{leader.topFinishes}</span></div>) : alltime.map((leader, index) => <div key={leader.username}><b>{index + 1}</b><span>{leader.username}</span><span>{leader.averagePercent.toFixed(1)}%</span><span>{leader.normalizedPerformance.toFixed(1)}</span><span>{leader.games}</span><strong>{leader.rating.toFixed(1)}</strong></div>)}
