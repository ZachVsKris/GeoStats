...
class SupabaseWarehouse:
    ...
    def finalize_v16_catalog(self, *, release_version: str = "16.2.2") -> Any:
        """Publish only through the guarded v16.2.x finalizer for the requested release."""
        if release_version not in {"16.2.1", "16.2.2"}:
            raise ValueError(f"Unsupported guarded catalog release: {release_version}")
        guard = "assert_v16_2_2_source_recovery" if release_version == "16.2.2" else "assert_v16_2_1_source_recovery"
