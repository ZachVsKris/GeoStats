...
check(pkg.version === "16.2.0" && version.includes('APP_VERSION = "16.2.0"'), "v16.2 version mismatch");
check(version.includes('RULES_VERSION = "16.2"'), "v16.2 rules version missing");
for (const file of [
...
check(supabase.includes("rpc/finalize_v16_2_catalog"), "finalizer does not call v16.2");

const exportAudit = read("scripts/export-v16-2-category-audit.py");
check(exportAudit.includes("category-promotion-dry-run-v16-2.csv") && exportAudit.includes("category-promotion-final-v16-2.csv") && exportAudit.includes('"randomOnlyTier": False'), "v16.2 pre/final audit export or no-Random-only declaration missing");
...
check(workflow.includes("Recover v16.2 audited catalog") && workflow.includes("recover-world-bank-catalog.py --refresh-values"), "v16.2 recovery workflow missing World Bank recovery");
...
check(workflow.includes("geostats-v16-2-category-audit") && workflow.includes("export-v16-2-category-audit.py --phase pre") && workflow.includes("export-v16-2-category-audit.py --phase final"), "v16.2 pre/final audit artifact missing");
check(workflow.includes("actions/upload-artifact@v4"), "workflow uses a deprecated artifact action");
const verifyWorkflow = read(".github/workflows/verify-v16.yml");
check(verifyWorkflow.includes("Verify GeoStats v16.2") && verifyWorkflow.includes("npm run test-v16-2"), "verification workflow still targets an older release");
...
if (failures.length) {
  console.error("GeoStats v16.2 checks FAILED:\n" + failures.map((failure) => ` - ${failure}`).join("\n"));
  process.exit(1);
}
console.log("GeoStats v16.2 static checks passed.");
