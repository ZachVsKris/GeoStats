...
class Importer(WarehouseImporter):
 ...
 def discover(self):
  ...
  for key,(title,desc,icon) in SPECS.items():
   ...
   out.append(CandidateDefinition(rule,f'FISHSTAT:{key}',title,SOURCE_PAGE,{'source_page_url':SOURCE_PAGE,'methodology_url':SOURCE_PAGE,'broadDomain':'agriculture','knowledgeCluster':'fisheries-production','strategyFamily':key,'boardDescription':desc}))
  return out
 def fetch_observations(self,c):
  ...
  for i,y,cap,aqua in self.rows:
   ...
   if i not in by or y>by[i][0]:by[i]=(y,value)
