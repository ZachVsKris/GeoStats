...
const versionParts=String(pkg.version||'0.0.0').split('.').map((part)=>Number(part)||0);
const atLeastV1626=versionParts[0]>16 || (versionParts[0]===16 && (versionParts[1]>2 || (versionParts[1]===2 && versionParts[2]>=6)));
check(atLeastV1626,`package version ${pkg.version} predates 16.2.6`);
check(version.includes(`APP_VERSION = \"${pkg.version}\"`) && version.includes(`RULES_VERSION = \"${pkg.version}\"`),'app/rules version mismatch');
...
const seededAuthCall=seeded.indexOf('await internalTesterAccess()');
const seededCatalogLoad=seeded.indexOf('await Promise.all([fetchCountries(), loadPuzzleCatalogSnapshot()])');
check(seededAuthCall>=0 && seededCatalogLoad>=0 && seededAuthCall<seededCatalogLoad,'Random authorization must fail closed before catalog loading/generation');
