...
export async function GET(request: Request) {
  ...
  if (!new Set(["all", "playable", "auto_promote", "manual_review", "rewrite_required", "data_repair_required", "duplicate", "excluded"]).has(promotion)) {
    return NextResponse.json({ error: "Unsupported v16.2 promotion filter." }, { status: 400 });
  }
    ...
    return NextResponse.json(
      {
        error: migrationMissing
          ? "Run RUN_THIS_IN_SUPABASE_FOR_V16_2.sql before using the Category Review Workbench."
          : error.message,
