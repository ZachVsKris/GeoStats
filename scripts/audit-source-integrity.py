...
SOURCE_SPECS: dict[str, tuple[str, str, str, str | None]] = {
    ...
    "constitute": ("import-historical-categories.py", "ConstituteImporter", "Constitute Project", "history:oldest-current-constitution"),
    "ipu": ("import-historical-categories.py", "IPUHistoricalImporter", "Inter-Parliamentary Union", "history:ipu-"),
    "worldbankhistory": ("import-historical-categories.py", "WorldBankHistoricalMilestonesImporter", "World Bank", "history:worldbank-"),
    "unwpp": ("import-un-wpp.py", "Importer", "United Nations Population Division", "unwpp:"),
    "worldbankclimate": ("import-world-bank-climate.py", "Importer", "World Bank Climate Change Knowledge Portal", "worldbankclimate:"),
    "naturalearthcapitals": ("import-natural-earth-capitals.py", "Importer", "Natural Earth", "natural-earth-capital:"),
    "imfweo": ("import-imf-weo.py", "Importer", "International Monetary Fund", "imfweo:"),
    "unescoich": ("import-unesco-ich.py", "Importer", "UNESCO", "unescoich:"),
    "noaatsunami": ("import-noaa-tsunami.py", "Importer", "NOAA National Centers for Environmental Information", "noaatsunami:"),
    "aquastat": ("import-aquastat.py", "Importer", "FAO AQUASTAT", "aquastat:"),
    "faofisheries": ("import-fao-fisheries.py", "Importer", "FAO Fisheries", "faofisheries:"),
    "usgsminerals": ("import-usgs-minerals.py", "Importer", "USGS Minerals", "usgsminerals:"),
    "whoghed": ("import-who-ghed.py", "Importer", "World Health Organization", "whoghed:"),
    "undesamigrant": ("import-un-desa-migrant-stock.py", "Importer", "United Nations Population Division", "undesamigrant:"),
    ...
    # official bulk inputs are unavailable.
    "globalfindex2025": ("import-global-findex.py", "Importer", "World Bank", "global-findex:"),
    "faofra2025": ("import-fao-fra-2025.py", "Importer", "FAO", "fao-fra-2025:"),
    "unicefdata": ("import-unicef-data.py", "Importer", "UNICEF", "unicef:"),
    "undphdr": ("import-undp-hdr.py", "Importer", "UNDP", "undp-hdr:"),
    "vdemv16": ("import-vdem-v16.py", "Importer", "V-Dem Institute", "vdem-v16:"),
    "faostatfoodsecurity": ("import-faostat-food-security.py", "Importer", "FAO", "faostat-food-security:"),
    "koppengeiger": ("import-koppen-geiger.py", "Importer", "Beck et al.", "koppen-geiger:"),
    "worldbankinfra": ("import-world-bank-infrastructure.py", "Importer", "World Bank WDI Infrastructure & Connectivity", "worldbankinfra:"),
    "faostatlanduse": ("import-faostat-land-use.py", "Importer", "FAOSTAT Land Use", "faostat-land-use:"),
    "faostatworldcover": ("import-faostat-worldcover.py", "Importer", "FAOSTAT / ESA WorldCover 2021", "faostat-worldcover:"),
    "worldbankwbl": ("import-world-bank-wbl.py", "Importer", "World Bank Women, Business and the Law 2026", "worldbank-wbl:"),
    "jmpwash": ("import-jmp-wash.py", "Importer", "WHO/UNICEF Joint Monitoring Programme", "jmp-wash:"),
    "unwup2025": ("import-un-wup-2025.py", "Importer", "United Nations Department of Economic and Social Affairs, Population Division", "unwup2025:"),
    "unwupcities2025": ("import-un-wup-2025-cities.py", "Importer", "United Nations Department of Economic and Social Affairs, Population Division", "unwupcities2025:"),
}

SOURCE_INPUT_ENV = {
    "imfweo": "IMF_WEO_INPUT",
    "noaatsunami": "NOAA_TSUNAMI_INPUT",
    "aquastat": "AQUASTAT_INPUT",
    "faofisheries": "FAO_FISHERIES_INPUT",
    "usgsminerals": "USGS_MINERALS_INPUT",
    "whoghed": "WHO_GHED_INPUT",
    "undesamigrant": "UN_DESA_MIGRANT_INPUT",
    ...
    "unicefdata": "UNICEF_DATA_INPUT",
    "undphdr": "UNDP_HDR_INPUT",
    "vdemv16": "VDEM_V16_INPUT",
    "faostatfoodsecurity": "FAOSTAT_FOOD_SECURITY_INPUT",
    "faostatlanduse": "FAOSTAT_LAND_USE_INPUT",
    "faostatworldcover": "FAOSTAT_WORLDCOVER_INPUT",
    "worldbankwbl": "WORLD_BANK_WBL_INPUT",
    "jmpwash": "JMP_WASH_INPUT",
    "unwup2025": "UN_WUP_2025_INPUT",
    "unwupcities2025": "UN_WUP_CITIES_2025_INPUT",
}
...
def classify_nonblocking_audit_result(result: Any) -> Any:
    ...
    critical_identity = {
        ...
        "ipu_endpoint", "ipu_field_identified", "universal_national_filter", "post_1940_scope",
        "query_identifies_series", "threshold_identified", "consecutive_crossing_rule",
    }
...
def audit_source(slug: str, warehouse: SupabaseWarehouse, *, include_nonplayable: bool = False) -> dict[str, Any]:
    ...
    input_env = SOURCE_INPUT_ENV.get(slug)
    if slug == "koppengeiger":
        raster_path = (os.environ.get("KOPPEN_GEIGER_RASTER_INPUT") or "").strip()
        countries_path = (os.environ.get("KOPPEN_GEIGER_COUNTRIES_INPUT") or "").strip()
        if not raster_path or not countries_path:
            raise RuntimeError(
                "koppengeiger independent audit requires KOPPEN_GEIGER_RASTER_INPUT and "
                "KOPPEN_GEIGER_COUNTRIES_INPUT pointing to the pinned official raster and sovereign geometry inputs."
            )
        importer = importer_class(warehouse, raster_path, countries_path, dry_run=True)
    elif input_env:
        input_path = (os.environ.get(input_env) or "").strip()
        if not input_path:
            raise RuntimeError(f"{slug} independent audit requires {input_env} pointing to the same official bulk file/URL family used for import.")
        importer = importer_class(warehouse, input_path, dry_run=True)
    else:
