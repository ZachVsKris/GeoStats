...
function blockerTone(category: CategoryRow) {
  if (category.computed_playable_v15) return "playable";
...
export default function CategoryReviewWorkbench() {
  ...
  const loadQueue = useCallback(async (preferredId?: string | null) => {
    ...
    setLoadingQueue(false);
  }, [committedQuery, source, status, readiness, sort, page, selectedId]);
  ...
  useEffect(() => {
    void loadQueue();
  }, [committedQuery, source, status, readiness, sort, page]); // eslint-disable-line react-hooks/exhaustive-deps
      ...
      <article className="reviewDetailPane">
        ...
        {current && <>
          <div className="reviewDetailHeader">
            ...
            <div className={`reviewReadinessBadge ${current.computed_playable_v15 ? "playable" : current.hard_gate_ready ? "ready" : "blocked"}`}>
              {current.computed_playable_v15 ? "Playable" : current.hard_gate_ready ? "Integrity-ready" : "Hard blockers"}
            </div>
          </div>
          ...
          <div className="reviewDetailGrid">
            <section className="reviewPanel">
              ...
              {current.auto_vetting_recommendation
                ...
                : <p className="reviewFinePrint">Run the v15.8 expanded-catalog vetting workflow after imports. Recommendations never activate categories automatically.</p>}
            </section>
            <section className="reviewPanel">
            ...
            <section className="reviewPanel">
              <h3>Hard integrity gate</h3>
              {current.hard_gate_ready
                ? <div className="reviewPass"><strong>Passes automated integrity requirements.</strong><span>Editorial approval can make it playable.</span></div>
                : <div className="reviewBlockerList">{(current.v15_blockers ?? []).filter((blocker) => !blocker.startsWith("Editorial") && !blocker.startsWith("Flagged") && !blocker.startsWith("Marked")).map((blocker) => <div key={blocker}>{blocker}</div>)}</div>}
          ...
          <div className="reviewDetailGrid values">
            <section className="reviewPanel">
              ...
              {!loadingDetail && <ol className="reviewRankingList">{(detail?.top ?? []).map((row) => <li key={row.country_iso3}><span>{row.country_name}</span><strong>{formatValue(row.value, current.unit)}</strong></li>)}</ol>}
            </section>
            <section className="reviewPanel">
              <h3>Opposite end</h3>
              {!loadingDetail && <ol className="reviewRankingList reverse">{(detail?.bottom ?? []).map((row) => <li key={row.country_iso3}><span>{row.country_name}</span><strong>{formatValue(row.value, current.unit)}</strong></li>)}</ol>}
            </section>
          </div>
