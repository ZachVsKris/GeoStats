...
export async function loadServerWarehousePayload(
  ...
  if (categoryResult.error) {
    const missingMigration =
      /category_runtime_review_v16|does not exist|schema cache/i.test(
        categoryResult.error.message,
      );

    throw new WarehouseCategoryError(
      missingMigration
        ? "The v16 category catalog is not installed. Run RUN_THIS_IN_SUPABASE_FOR_V16_0.sql."
        : categoryResult.error.message,
      500,
    );
  }
  ...
  if (category.computed_playable_v16 !== true) {
    throw new WarehouseCategoryError(
