...
export default function AccountControls({
  ...
  async function loadProfile(fallbackEmail?: string | null) {
    ...
    try {
      ...
      if (profile.usernameCustomized === false) {
        setMessage("Choose a GeoStats username for your account.");
        setOpen(true);
      }
  ...
  async function saveUsername() {
    ...
    try {
      ...
      setMessage("Username saved to your account.");
  ...
  return <>
    <div className={results ? "resultsAccountActions" : "accountHeaderActions"}>
      {results && userLabel && <a href="/history">My results</a>}
    ...
    {open && <div className="modal accountModal" onClick={(event) => event.currentTarget === event.target && usernameCustomized && setOpen(false)}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={`account-dialog-title-${context}`} aria-describedby={`account-dialog-description-${context}`} aria-busy={saving || savingUsername || sendingLink || signingInWithGoogle}>
        ...
        {userLabel ? <>
          ...
          <div className="accountModalActions"><a href="/history" className="accountHistoryLink">My results and stats</a><button type="button" onClick={saveUsername} disabled={savingUsername || usernameDraft.length < 3 || usernameDraft === username}>{savingUsername ? "Saving…" : usernameCustomized ? "Update username" : "Save username"}</button><button type="button" className="quietButton" onClick={signOut}>Sign out</button></div>
          <p id={`account-dialog-description-${context}`}>Your completed Dailies are saved to your account automatically. Your email stays private.</p>
          {saving && <p>Saving your completed Daily…</p>}
        </> : <>
          <p id={`account-dialog-description-${context}`}>Sign in to save your completed Dailies across devices. Every mode is free to play without an account.</p>
          <ul className="accountBenefits">
            <li>Keep your complete Daily history across devices</li>
            <li>See your personal averages and best scores</li>
