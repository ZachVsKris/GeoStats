...
type CategoryRow = {
  ...
  promotion_decision_v16_2?: "playable" | "auto_promote" | "manual_review" | "rewrite_required" | "data_repair_required" | "duplicate" | "excluded" | null;
...
type IntegritySourceRow = { source: string; categories: number; playable: number; verified: number; verified_with_warnings: number; blocked: number; audit_pending: number; last_validated_at: string | null };
type IntegrityIssue = { id: string; title: string; source_organization: string; integrity_state: string; validation_status: string; validation_reason: string | null; validated_at: string | null; validation_mismatch_count: number; validation_ranking_mismatch_count: number };
...
type IntegrityOverview = { enforcement_enabled: boolean; categories: number; playable: number; verified: number; verified_with_warnings: number; blocked: number; audit_pending: number; unverified_playable: number };
...
const REPO_ACTIONS = "https://github.com/ZachVsKris/geostats/actions/workflows";
const WORKFLOWS: Record<string, string> = {
  historical: `${REPO_ACTIONS}/import-historical-v16-2-5.yml`,
  ...
  automaticExpansion: `${REPO_ACTIONS}/import-v16-expansion.yml`,
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setGenerationElapsed(Math.floor((Date.now() - started) / 1000)), 1000);
  ...
  const filtered = useMemo(() => {
    ...
    const rows = data?.categories.filter((category) =>
      ...
      && (promotion === "all" || category.promotion_decision_v16_2 === promotion || category.blocker_class_v16_2 === promotion)
  ...
  }, [data, query, source, review, promotion, minimumCoverage, sort]);
  ...
  async function generateBoards() {
    ...
    try {
      ...
      setError(diagnostics
        ? `${errorWithDiagnostics.message} Stage: ${diagnostics.failureStage ?? "unknown"}; eligible datasets: ${diagnostics.eligibleDatasets ?? "—"}; candidates: ${Object.values(diagnostics.validCandidates ?? {}).join("/") || "—"}; compatible pairs: ${diagnostics.compatiblePairs ?? 0}; joint attempts: ${diagnostics.jointConstructionAttempts ?? 0}; elapsed: ${Math.round((diagnostics.elapsedMs ?? generationElapsed * 1000) / 1000)}s.${diagnostics.lastTrioErrors?.length ? ` Conflicts: ${diagnostics.lastTrioErrors.slice(0, 3).join("; ")}` : ""}`
  ...
  const approvedButBlocked = data.categories.filter((category) => category.editorial_status === "approved" && !category.computed_playable).length;
  const promotionCounts = data.categories.reduce<Record<string, number>>((counts, category) => {
    ...
    <>
      ...
      <section style={{ ...card, marginBottom: 16, borderColor: "rgba(185,244,90,.45)", background: "linear-gradient(135deg,rgba(38,78,50,.72),rgba(8,30,24,.8))" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          <div><span className="kicker">v16.2.5 catalog recovery</span><h2 style={{ margin: "5px 0" }}>Review categories in the Workbench</h2><p style={{ margin: 0, opacity: .72 }}>One authoritative decision, permanent political/self-report and clarity flags, source values, overlaps, and keyboard review.</p></div>
      ...
      <section className="adminStatGrid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12 }}>
        {[
          ...
          ["Playable", data.integrity.overview.playable],
          ["Approved but blocked", approvedButBlocked],
          ["Manual review", promotionCounts.manual_review ?? 0],
          ["Needs rewrite", promotionCounts.rewrite_required ?? 0],
          ["Data repair", promotionCounts.data_repair_required ?? 0],
        ].map(([label, value]) => (
        ...
        {!data.integrity.migrationApplied ? <p style={{ marginBottom: 0 }}>Apply the v14.4 Supabase installer to enable integrity and board-quality reporting.</p> : <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(145px,1fr))", gap: 9, margin: "14px 0" }}>
            {[
              ...
              ["Verified with warnings", data.integrity.overview.verified_with_warnings],
              ["Blocked", data.integrity.overview.blocked],
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}>
            <thead><tr>{["Source", "Categories", "Playable", "Verified", "Warnings", "Blocked", "Re-audit due", "Last audit"].map((heading) => <th key={heading} style={{ textAlign: "left", padding: 8, borderBottom: "1px solid rgba(255,255,255,.12)" }}>{heading}</th>)}</tr></thead>
          ...
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid rgba(255,255,255,.12)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 9, marginTop: 10 }}>
              <div style={{ padding: 11, borderRadius: 10, background: "rgba(255,255,255,.04)" }}><div style={{ opacity: .68, fontSize: 12 }}>Winner requirement</div><strong style={{ fontSize: 20 }}>Top {data.boardQuality.winnerGlobalRankLimit} · DATE exempt</strong><div style={{ opacity: .58, fontSize: 11, marginTop: 3 }}>Historical dates still require complete, trustworthy rankings.</div></div>
            ...
            {data.boardQuality.similarityConflicts.length > 0 && <div style={{ marginTop: 12 }}>
              <div style={{ opacity: .72, fontSize: 12, marginBottom: 5 }}>Different families whose wording is at or above the {Math.round(data.boardQuality.semanticSimilarityThreshold * 100)}% review threshold. These are warnings only unless structured semantic metadata also identifies a real gameplay conflict.</div>
      ...
      <section style={{ ...card, marginTop: 16 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          ...
          <button style={button} disabled={generating || boardsLocked} onClick={generateBoards}>
            {generating ? `Generating… ${generationElapsed}s` : boardsLocked ? "Locked after first score" : "Generate Daily trio"}
          </button>
        </div>
        {generating && <div style={{ marginTop: 10, opacity: .72, fontSize: 13 }}>Bounded interactive search; this request stops after about one minute. Deeper retries run through the scheduled generator.</div>}
        {generation && (
      ...
      <section style={{ marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: 16, flexWrap: "wrap", marginBottom: 10 }}>
          ...
          <a href={WORKFLOWS.automaticExpansion} target="_blank" rel="noreferrer" style={{ ...button, textDecoration: "none" }}>Run v16.2.5 catalog recovery ↗</a>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(230px,1fr))", gap: 12 }}>
          {data.sources.filter((item) => item.metadata?.retired !== true).map((item) => {
            ...
            return (
              <article key={item.id} style={card}>
                ...
                {key === "world-bank" || key === "worldbank" ? (
                  ...
                  <a href={WORKFLOWS.automaticExpansion} target="_blank" rel="noreferrer" style={{ ...button, display: "inline-block", textDecoration: "none" }}>Run selected UNESCO repairs ↗</a>
                ) : key === "ilostat" ? (
                ...
                ) : ["unmembership", "constitute", "ipu"].includes(key) ? (
                  <a href={WORKFLOWS.historical} target="_blank" rel="noreferrer" style={{ ...button, display: "inline-block", textDecoration: "none" }}>Imported through v16.2.5 historical workflow ↗</a>
                ) : ["pewreligion", "faostatfbs", "smithsoniangvp", "usgs", "naturalearth", "unescoheritage", "untourism"].includes(key) ? (
                  <a href={WORKFLOWS.automaticExpansion} target="_blank" rel="noreferrer" style={{ ...button, display: "inline-block", textDecoration: "none" }}>Run v16.2.5 recovery ↗</a>
                ) : ["aquastat", "usgsminerals", "faofisheries", "worldcover", "hydrosheds", "elevation"].includes(key) ? (
                  <button style={mutedButton} disabled>Deferred until fully automatic</button>
                ) : (
                  <button style={mutedButton} disabled>Automatic importer not available</button>
                )}
              </article>
            );
          })}
      ...
      <section style={{ ...card, marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: 16, flexWrap: "wrap" }}>
          <div>
            ...
            <p style={{ opacity: .72, marginBottom: 0 }}>Daily and Random use one shared strict-pass playable catalog. There is no Random-only tier.</p>
          </div>
          <strong>{formatNumber(filtered.length)} shown</strong>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "minmax(220px,2fr) repeat(5,minmax(135px,1fr))", gap: 10, marginTop: 16 }}>
          ...
          <select style={input} value={promotion} onChange={(event: ChangeEvent<HTMLSelectElement>) => setPromotion(event.target.value)}>
            <option value="all">All catalog outcomes</option>
            ...
            <option value="source_specific_quality">Quality-only blocker</option>
            <option value="substantive_data_failure">Data-integrity blocker</option>
          </select>
            ...
            <tbody>
              {filtered.map((category) => (
                <tr key={category.id} style={{ borderBottom: "1px solid rgba(255,255,255,.07)" }}>
                  ...
                  <td style={{ padding: 8, maxWidth: 280 }}>{category.primary_blocker_v16_2 ?? (category.computed_playable ? "Passes all current gameplay gates." : category.playability_blockers?.[0] ?? "No primary blocker recorded.")}</td>
