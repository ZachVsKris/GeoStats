...
from data_pipeline.editorial_curation import FAOSTAT_ALLOWED
...
LIVESTOCK_POPULATION_ELEMENTS = {"stocks"}
LIVESTOCK_ITEM_TOKENS = (
    "cattle", "buffalo", "sheep", "goat", "pig", "swine", "chicken", "poultry",
    "duck", "turkey", "camel", "horse", "ass", "mule", "rabbit", "beehive",
    "livestock", "animal",
)
...
def livestock_population_allowed(item: str, element: str, unit: str) -> bool:
    """Keep intuitive animal-population totals, but not productivity inputs."""
    normalized_item = normalize_element(item)
    normalized_element = normalize_element(element)
    normalized_unit = normalize_element(unit)
    return (
...
def element_allowed(element: str, item: str = "", unit: str = "") -> bool:
    ...
    return (
        normalize_element(element) in ALLOWED_PRODUCTION_ELEMENTS
        or livestock_population_allowed(item, element, unit)
    )
...
def category_title(
    ...
    curated = FAOSTAT_ALLOWED.get(f"QCL:'{item_code}:{element_code}") if item_code and element_code else None
    if curated:
        return curated
    if element_code == "5111" and item_code == "F1746":
        return "Largest combined cattle and buffalo population"
    if element_code == "5111" and item_code == "F1749":
        return "Largest combined sheep and goat population"
    if normalize_element(element) in ALLOWED_PRODUCTION_ELEMENTS:
        return f"Most {item_clean[:1].lower() + item_clean[1:]} produced"
    if livestock_population_allowed(item, element, unit):
        normalized_item = normalize_element(item_clean)
        label = LIVESTOCK_SINGULAR_LABELS.get(normalized_item, normalized_item)
        return f"Largest {label} population"
        ...
        with archive.open(member) as binary:
            ...
            for row_number, row in enumerate(reader, start=2):
                ...
                if not item_code or not item or not element_code or not unit:
                    continue
                if not element_allowed(element, item, unit):
                    continue
        ...
        candidates.append(
            {
                ...
                "title": category_title(item, element, unit, item_code, element_code),
                "description": (
                    f"Total national production of {item}."
                    if normalize_element(element) in ALLOWED_PRODUCTION_ELEMENTS
                    else f"Total national population of {item}."
                ),
...
def validate_faostat_categories(
    ...
    for candidate in candidates:
        ...
        if "yield" in element_lower:
            ...
            measure_semantics = ("produced" in title_lower or "production" in title_lower) and "yield" not in title_lower
        elif "stocks" in element_lower or str(candidate.get("element_code") or "") == "5111":
            measure_semantics = ("population" in title_lower or "stocks" in title_lower) and str(candidate.get("unit") or "").lower() == "animals"
        ...
        checksum_only_warning = source_checksum != stored_checksum and not reasons
