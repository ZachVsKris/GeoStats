...
function catalogError(message: string) {
  if (/schema cache/i.test(message)) {
    return new Error("Supabase has not refreshed its REST schema cache for the v16.2.6 catalog. Run NOTIFY pgrst, 'reload schema'; and retry after 30 seconds.");
  }
  if (/category_runtime_review_v16_2|does not exist|relation .* not found/i.test(message)) {
    return new Error("The GeoStats v16.2.6 catalog migration is not installed. Run RUN_THIS_IN_SUPABASE_FOR_V16_2_6.sql.");
  }
...
const loadCachedPlayableRows = unstable_cache(
  ...
  ["geostats-playable-category-rows-v16.2.6-random-hotfix1"],
  { revalidate: 300, tags: ["geostats-playable-category-catalog"] },
);

const loadCachedRegistryRows = unstable_cache(
  () => loadRows(),
  ["geostats-category-registry-rows-v16.2.6-random-hotfix1"],
  { revalidate: 300, tags: ["geostats-playable-category-catalog"] },
);
