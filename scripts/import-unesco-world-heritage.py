...
def make_rule(key,title,description,icon='🏛️'):
 return IndicatorRule(key=key,title=title,description=description,plain_language_description=description,
  ...
  unit_explanation='inscribed properties',family='Culture',icon=icon,unit='sites',value_type='total',ranking_direction='high',
...
class Importer(WarehouseImporter):
 ...
 def discover(self):
  return [CandidateDefinition(rule=r,source_indicator_code=f'WHC:{r.key}',source_indicator_name=r.title,source_url=SOURCE_PAGE,
   metadata={'source_page_url':SOURCE_PAGE,'methodology_url':METHODOLOGY,'dataset_release':f'UNESCO World Heritage List {YEAR}','minimum_year':1978,
   'broadDomain':'culture','knowledgeCluster':'world-heritage','strategyFamily':'world-heritage','boardDescription':r.description}) for r in RULES]
