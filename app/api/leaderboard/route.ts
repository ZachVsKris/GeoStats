...
import { BOARD_NORMALIZATION_VERSION, LEADERBOARD_RATING_VERSION } from "../../../lib/version";
...
export async function GET(request: Request) {
  ...
  let { data, error } = await buildQuery("user_id,challenge_date,difficulty,score,average_placement,firsts,top_fives,board_normalization_version,leaderboard_rating_version,profiles(username,display_name)");
  if (error && (error.code === "42703" || error.code === "PGRST204" || /normalization_version|rating_version/i.test(error.message ?? ""))) {
    const legacy = await buildQuery("user_id,challenge_date,difficulty,score,average_placement,firsts,top_fives,profiles(username,display_name)");
    data = legacy.data;
    error = legacy.error;
  }
