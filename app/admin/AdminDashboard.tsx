...
type TopCategoryExposure = { category_id: string; category_title: string; appearances: number; first_date: string; last_date: string };
type CountryExposureSummary = { daily_dates: number; country_slots: number; distinct_countries: number; max_country_appearances: number | null; median_repeat_interval_days: number | null };
    ...
    <>
      ...
      <section style={{ ...card, marginBottom: 16, borderColor: "rgba(185,244,90,.45)", background: "linear-gradient(135deg,rgba(38,78,50,.72),rgba(8,30,24,.8))" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          <div><span className="kicker">v16.2.6 catalog recovery</span><h2 style={{ margin: "5px 0" }}>Review categories in the Workbench</h2><p style={{ margin: 0, opacity: .72 }}>One authoritative decision, permanent political/self-report and clarity flags, source values, overlaps, and keyboard review.</p></div>
      ...
      <section style={{ ...card, marginTop: 16 }}>
        <h2 style={{ marginTop: 0 }}>Daily diversity · last 30 dates</h2>
        {!data.diversity.migrationApplied ? (
          <div style={{ opacity: .72 }}>Run the v16.2.6 Supabase migration to enable category and country exposure diagnostics.</div>
        ) : (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10 }}>
      ...
      <section style={{ marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: 16, flexWrap: "wrap", marginBottom: 10 }}>
          ...
          <a href={WORKFLOWS.automaticExpansion} target="_blank" rel="noreferrer" style={{ ...button, textDecoration: "none" }}>Run v16.2.6 catalog recovery ↗</a>
        </div>
            ...
            return (
              <article key={item.id} style={card}>
                ...
                {key === "world-bank" || key === "worldbank" ? (
                  ...
                  <a href={WORKFLOWS.historical} target="_blank" rel="noreferrer" style={{ ...button, display: "inline-block", textDecoration: "none" }}>Imported through v16.2.6 historical workflow ↗</a>
                ) : ["pewreligion", "faostatfbs", "smithsoniangvp", "usgs", "naturalearth", "unescoheritage", "untourism"].includes(key) ? (
                  <a href={WORKFLOWS.automaticExpansion} target="_blank" rel="noreferrer" style={{ ...button, display: "inline-block", textDecoration: "none" }}>Run v16.2.6 recovery ↗</a>
