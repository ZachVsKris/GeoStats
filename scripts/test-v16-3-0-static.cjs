...
check(accounts.includes("keepFocusInside") && accounts.includes('role="status"') && accounts.includes("Google sign-in is temporarily unavailable") && !accounts.includes("googleProviderIsEnabled") && !accounts.includes("googleAvailable"), "account modal resilience/accessibility/provider guard is incomplete");
