...
export function inferSemanticProfile(category: Category): CategorySemanticProfile {
  ...
  if (category.source === "unhcr" || contains(text, [/refugee/, /asylum/, /displacement/])) {
    if (contains(text, [/origin/, /originating/, /by-origin/])) {
      return { family: "forced-displacement-origin", topic: slug(category.similarityGroup || category.id), broadDomain: "displacement" };
    }
    if (contains(text, [/hosted/, /received/, /destination/, /receiving/])) {
      return { family: "forced-displacement-destination", topic: slug(category.similarityGroup || category.id), broadDomain: "displacement" };
    }
    return { family: "forced-displacement", topic: slug(category.similarityGroup || category.id), broadDomain: "displacement" };
  }
  ...
  if (contains(text, [/employment-to-population/, /employment-population/, /unemployment/, /labor-force-participation/, /labour-force-participation/])) {
    return { family: "labor-market-utilization", topic: slug(category.similarityGroup || category.id), broadDomain: "labor" };
  }
  if (contains(text, [/labor-productivity/, /labour-productivity/, /productivity-growth/])) {
    return { family: "labor-productivity", topic: slug(category.similarityGroup || category.id), broadDomain: "labor" };
  }
  if (contains(text, [/self-employment/, /wage-employment/, /employment-status/])) {
    return { family: "employment-status", topic: slug(category.similarityGroup || category.id), broadDomain: "labor" };
  }

  if (category.source === "faostat") {
    if (contains(text, [/(^|-)yield($|-)/])) return { family: "crop-yield", topic: slug(category.similarityGroup || category.id), broadDomain: "agriculture" };
    if (contains(text, [/(^|-)production($|-)/])) return { family: "crop-production", topic: slug(category.similarityGroup || category.id), broadDomain: "agriculture" };
    if (contains(text, [/area-harvested/, /harvested-area/])) return { family: "crop-harvested-area", topic: slug(category.similarityGroup || category.id), broadDomain: "agriculture" };
  }

  if (contains(text, [/gdp/, /gross-domestic-product/, /economic-output/])) {
    return { family: "economic-output", topic: slug(category.similarityGroup || category.id), broadDomain: "economy" };
  }
  if (contains(text, [/forest-area/, /forest-cover/, /forest-percent/, /forest-share/, /least-forest/])) {
    return { family: "forest-cover", topic: slug(category.similarityGroup || category.id), broadDomain: "environment" };
  }
  if (contains(text, [/urban-population/, /rural-population/, /urbanization/, /settlement-share/])) {
    return { family: "settlement-share", topic: slug(category.similarityGroup || category.id), broadDomain: "population" };
  }
  if (contains(text, [/life-expectancy/])) return { family: "life-expectancy", topic: slug(category.similarityGroup || category.id), broadDomain: "health" };
  if (contains(text, [/infant-mortality/])) return { family: "infant-mortality", topic: slug(category.similarityGroup || category.id), broadDomain: "health" };
  if (contains(text, [/maternal-mortality/])) return { family: "maternal-mortality", topic: slug(category.similarityGroup || category.id), broadDomain: "health" };
  if (contains(text, [/vaccination/, /immunization/, /measles-vaccine/])) return { family: "immunization-coverage", topic: slug(category.similarityGroup || category.id), broadDomain: "health" };
  if (contains(text, [/merchandise-export/, /general-export/, /exports-share/])) return { family: "general-exports", topic: slug(category.similarityGroup || category.id), broadDomain: "trade" };
  if (contains(text, [/merchandise-import/, /general-import/])) return { family: "general-imports", topic: slug(category.similarityGroup || category.id), broadDomain: "trade" };

  const fallback = category.similarityGroup || category.indicator || category.id;
  return { family: slug(fallback), topic: slug(fallback), broadDomain };
}
...
export function semanticConflict(first: Category, second: Category) {
  ...
  if (firstProfile.family === secondProfile.family) return true;
  return categorySemanticSimilarity(first, second) >= MAX_SAME_BOARD_SEMANTIC_SIMILARITY;
}
