  ...
  for (let from = 0; ; from += 1000) {
    ...
    if (error) {
      ...
      const guidance = /schema cache/i.test(message)
        ? "Supabase has not refreshed the v16.2.5 REST schema cache. Run NOTIFY pgrst, 'reload schema'; and retry after 30 seconds."
        : /category_runtime_review_v16_2|does not exist/i.test(message)
          ? "Run RUN_THIS_IN_SUPABASE_FOR_V16_2_5.sql before loading the v16.2.5 Admin dashboard."
          : message;
