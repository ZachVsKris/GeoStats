...
create table if not exists public.v15_7_curated_copy (
...
insert into public.v15_7_curated_copy(
...
values
  ('EN.URB.LCTY', 'Largest city by population', 'Population living in each country''s largest urban area.', now()),
  ('AG.LND.TOTL.K2', 'Largest land area', 'Total land area within the country''s borders.', now()),
  ('EN.GHG.ALL.MT.CE.AR5', 'Most greenhouse gas emissions', 'Total emissions excluding land use and forestry.', now()),
  ('EN.GHG.CO2.PC.CE.AR5', 'Most CO₂ emissions per person', 'Average carbon dioxide emissions for each person.', now()),
  ('EN.ATM.PM25.MC.M3', 'Highest fine-particle air pollution', 'Average exposure to fine airborne particles.', now()),
  ('BX.GSR.CMCP.ZS', 'Highest communications and IT export share', 'Communications and IT services as a share of service exports.', now()),
  ('BM.GSR.CMCP.ZS', 'Highest communications and IT import share', 'Communications and IT services as a share of service imports.', now()),
  ('BX.GSR.TRAN.ZS', 'Highest transport share of service exports', 'Transport services as a share of service exports.', now()),
  ('BM.GSR.TRAN.ZS', 'Highest transport share of service imports', 'Transport services as a share of service imports.', now()),
  ('BX.GSR.TRVL.ZS', 'Highest travel share of service exports', 'Foreign visitor spending as a share of service exports.', now()),
  ('BM.GSR.TRVL.ZS', 'Highest travel share of service imports', 'Resident spending abroad as a share of service imports.', now())
on conflict (source_indicator_code) do update
set
  player_title = excluded.player_title,
  board_description = excluded.board_description,
  updated_at = now();
...
set metadata = coalesce(category.metadata, '{}'::jsonb)
    ...
    updated_at = now()
from public.v15_7_curated_copy copy
...
set status = 'approved',
    ...
    updated_at = now()
from public.stat_categories category
join public.v15_7_curated_copy copy on copy.source_indicator_code = category.source_indicator_code
...
-- ---------------------------------------------------------------------------
-- 4A. Enforce the agreed FAOSTAT production-only policy.
--     Only QCL production elements 5510 and 5513 may remain playable.
--     Yield, harvested area, stocks, animal counts, slaughter, carcass and
--     efficiency measures are editorially retired even when their data validate.
-- ---------------------------------------------------------------------------
create table if not exists public.v15_7_faostat_nonproduction_decisions (
  category_id text primary key references public.stat_categories(id) on delete cascade,
...
insert into public.v15_7_faostat_nonproduction_decisions(
  category_id,
  source_indicator_code,
  title,
  element_code,
...
select
  ...
  'FAOSTAT non-production measure excluded by GeoStats policy. Only production elements 5510 and 5513 are playable.',
  now()
from public.stat_categories category
...
set
  ...
  notes = concat_ws(
    E'\n',
    nullif(review.notes, ''),
    'v15.7 production-only policy: FAOSTAT non-production element retired.'
  ),
  reviewed_at = coalesce(review.reviewed_at, now()),
  updated_at = now()
where review.category_id in (
  select category_id
  from public.v15_7_faostat_nonproduction_decisions
);
...
set
  ...
  preferred_category_id = null,
  decision_reason = 'FAOSTAT non-production element retired. GeoStats permits total production elements 5510 and 5513 only.',
  decision_source = 'v15.7 production-only policy',
  reviewed_at = now()
...
set
  ...
  metadata = coalesce(category.metadata, '{}'::jsonb)
    || jsonb_build_object(
      'catalogTier', 'quarantined',
      'faostatProductionOnlyV15_7', 'blocked-nonproduction',
      'faostatElementCodeV15_7',
        regexp_replace(
          regexp_replace(coalesce(category.source_indicator_code, ''), '^.*:', ''),
          '[^0-9]',
          '',
...
select count(*) as playable_faostat_nonproduction_categories
from public.stat_categories category
where lower(coalesce(category.source_organization, '')) like '%fao%'
  and coalesce(category.source_indicator_code, '') ~* '^QCL:'
  and regexp_replace(
