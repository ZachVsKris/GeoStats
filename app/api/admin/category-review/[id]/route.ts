...
async function loadDetail(admin: SupabaseClient, id: string) {
  ...
  if (categoryResult.error || !categoryResult.data) {
    return { categoryResult, detail: null };
  }
  ...
  let similarQuery = admin
    .from("category_review_queue_v15")
    .select(
      "id,effective_title,source_organization,source_indicator_code,effective_semantic_group,editorial_status,computed_playable_v15,duplicate_of",
    )
    .neq("id", id)
    .limit(16);

  if (semanticGroup) {
    similarQuery = similarQuery.eq("effective_semantic_group", semanticGroup);
  } else if (category.source_organization) {
  ...
  const [top, bottom, similar, history] = await Promise.all([
    ...
    admin
      ...
      .select(
        "id,previous_state,next_state,created_at,reviewer_user_id",
      )
      .eq("category_id", id)
...
export async function GET(
  ...
  if (loaded.categoryResult.error) {
    return NextResponse.json(
      { error: loaded.categoryResult.error.message },
      { status: 500 },
    );
  }

  if (!loaded.categoryResult.data) {
    return NextResponse.json(
  ...
  const detailError =
    loaded.detail && "error" in loaded.detail
      ? loaded.detail.error
      : null;
...
export async function POST(
  ...
  const body = (await request
    .json()
    .catch(() => ({}))) as ReviewUpdate;

  if (body.status !== undefined && !STATUSES.has(body.status)) {
    return NextResponse.json(
      { error: "Unsupported review status." },
      { status: 400 },
    );
  }
  ...
  if (duplicateOf === id) {
    return NextResponse.json(
      { error: "A category cannot duplicate itself." },
      { status: 400 },
    );
  }
  ...
  if (!previous) {
    return NextResponse.json(
      {
        error:
          "Category review state not found. Run the v15 SQL installer.",
      },
      { status: 409 },
    );
  }
  ...
  if (body.status !== undefined) {
    ...
    update.reviewed_by =
      body.status === "pending" ? null : auth.user.id;
  }
  ...
  if (body.recommended_title !== undefined) {
    update.recommended_title = recommendedTitle;
  }

  if (body.semantic_group !== undefined) {
    update.semantic_group = semanticGroup;
  }
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Choose the preferred category before marking this one duplicate.",
      },
      { status: 400 },
    );
  }
  ...
  if (body.status === "approved") {
    ...
    const blockingFlags = allowedBooleanKeys.filter((key) =>
      Boolean(merged[key]),
    );
    ...
    if (blockingFlags.length || merged.duplicate_of) {
      return NextResponse.json(
        {
          error:
            "Clear all blocking flags and the duplicate link before approving this category.",
          blockers: [
            ...blockingFlags,
  ...
  const { data: policy, error: policyError } = await auth.admin
    .from("category_review_queue_v15")
    .select(
      "id,editorial_status,computed_playable_v15,recommended_title,semantic_group",
    )
    .eq("id", id)
    .single();
  ...
  const legacyStatus =
    policy.editorial_status === "approved"
      ? "approved"
      : ["rejected", "duplicate"].includes(
            policy.editorial_status,
          )
        ? "rejected"
        : "needs_review";

  const legacyEditorialStatus =
    policy.editorial_status === "approved"
      ? "approved"
      : ["rejected", "duplicate"].includes(
            policy.editorial_status,
  ...
  const categoryUpdate: Record<string, unknown> = {
    ...
    curation_reason: `GeoStats v15 authoritative category review state: ${policy.editorial_status}.`,
    curation_version: "geostats-v15-review-v1",
    content_review_status: legacyEditorialStatus,
    content_review_reason: `GeoStats v15 authoritative category review state: ${policy.editorial_status}.`,
    ...
    player_quality_status:
      policy.editorial_status === "approved"
        ...
        : ["rejected", "duplicate"].includes(
          ...
          ? "blocked"
          : "caution",
    player_quality_reason: `GeoStats v15 authoritative category review state: ${policy.editorial_status}.`,
    enabled: Boolean(policy.computed_playable_v15),
    eligible_daily: Boolean(policy.computed_playable_v15),
  };

  if (
    typeof policy.semantic_group === "string" &&
    policy.semantic_group.trim()
  ) {
    categoryUpdate.semantic_family = policy.semantic_group.trim();
  }

  if (
    policy.computed_playable_v15 &&
    typeof policy.recommended_title === "string" &&
    policy.recommended_title.trim()
  ) {
    categoryUpdate.title = policy.recommended_title.trim();
    categoryUpdate.short_title = policy.recommended_title
      .trim()
      .slice(0, 70);
  }

  const { error: categoryError } = await auth.admin
