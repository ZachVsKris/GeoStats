...
type WorkbenchRow = {
  ...
  ranking_completeness_status: string;
  ranking_completeness_reason: string | null;
  top_value_feasible: boolean;
  top_value_distinct_count: number;
...
function approvalBlockers(category: WorkbenchRow, state: ReviewState): string[] {
  ...
  if (!category.hard_gate_ready) blockers.push("The category has not passed the numerical, source, or content hard gate.");
  if (!["comprehensive", "top_end_complete"].includes(category.ranking_completeness_status)) {
    blockers.push(category.ranking_completeness_reason || "Ranking completeness has not been established.");
  }
  if (!category.top_value_feasible) {
    blockers.push(`Only ${category.top_value_distinct_count ?? 0} distinct values are available among the meaningful top rankings.`);
  }
...
export async function POST(request: Request) {
  ...
  const [workbenchResult, stateResult] = await Promise.all([
    auth.admin
      ...
      .select("id,effective_title,editorial_status,hard_gate_ready,ranking_completeness_status,ranking_completeness_reason,top_value_feasible,top_value_distinct_count,v16_blockers")
      .in("id", categoryIds),
    auth.admin
  ...
  if (decision === "approved") {
    ...
    if (blocked.length) {
      return NextResponse.json({
        error: `${blocked.length} selected categor${blocked.length === 1 ? "y is" : "ies are"} not ready for approval under the v16 source, ranking-completeness, distinct-value, and editorial gates.`,
        blocked,
        missing,
      }, { status: 409 });
    }
