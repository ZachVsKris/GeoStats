...
export async function GET(
  ...
  try {
    ...
    await recordGeneration(supabase, {
      challenge_date: date,
      status: "failed",
      source: "daily-bootstrap-v15.9.2",
  ...
  if (validated.rounds.easy && validated.rounds.normal && validated.rounds.expert
    ...
    return NextResponse.json({
      found: true,
      generated: false,
      legacyModes: Object.keys(validated.outdated),
      warning: Object.keys(validated.outdated).length
  ...
  try {
    ...
    if (latestValidated.rounds.easy && latestValidated.rounds.normal && latestValidated.rounds.expert
      ...
      return NextResponse.json({
        found: true,
        generated: false,
        legacyModes: Object.keys(latestValidated.outdated),
        warning: Object.keys(latestValidated.outdated).length
          ? "This valid Daily board is preserved under the rules and category set with which it was generated."
          : undefined,
    ...
    const replaceable = new Set<DailyDifficulty>();
    ...
    for (const difficulty of DAILY_DIFFICULTIES) {
      ...
      if (round && !hardErrors.length) {
        // A mode that is individually valid is immutable during automatic repair,
        // whether or not it has already been scored. Missing or invalid modes are
        // generated around it; successful modes are never sacrificed to help a
        // different mode pass cross-mode composition.
    ...
    // If all three stored modes are individually valid, preserve them exactly.
    // A later cross-mode preference must not discard a successful unscored mode.
    // Selective migrations remove genuinely retired categories before this route
    // runs; remaining trio-only conflicts are treated as legacy diagnostics.
    if (fixed.easy && fixed.normal && fixed.expert) {
      return NextResponse.json({
        found: true,
        generated: false,
        legacyModes,
        warning: (legacyModes.length || hasTrioConflict)
          ? "This Daily trio is preserved under the rules with which its individual modes were generated."
          : undefined,
        trioDiagnostics: hasTrioConflict ? latestValidated.trioErrors : undefined,
        ...shape(latestBefore.rows, dependencies),
      }, { headers: { "Cache-Control": DAILY_SUCCESS_CACHE } });
    }
    ...
    for (let attempt = 0; attempt < 3 && !generated; attempt += 1) {
      try {
        generated = await generateDailyTrio(
          dependencies.countries,
          date,
          fixed,
          attempt === 0 ? "" : `preserve-valid-modes-${attempt + 1}`,
        );
      } catch (generationError) {
        lastGenerationError = generationError;
      }
    }
    if (!generated) throw lastGenerationError ?? new Error("The remaining Daily modes could not be generated.");
