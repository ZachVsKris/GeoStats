...
import { configForDifficultyDimensions, type DailyDifficulty } from "../../../lib/gameRules";
import { loadServerCategoryRegistry } from "../../../lib/serverPlayableCatalog";
import {
  BOARD_NORMALIZATION_VERSION,
  LEADERBOARD_RATING_VERSION,
  SCORING_VERSION,
  RULES_VERSION,
} from "../../../lib/version";
...
const legacyScoreColumns = "score,average_placement,firsts,top_fives,difficulty,assignments,completed_at";
const scoreColumns = `${legacyScoreColumns},scoring_version,board_normalization_version,leaderboard_rating_version,rules_version,category_set_version,dataset_version`;

function missingVersionColumns(error: { code?: string; message?: string } | null | undefined) {
  return Boolean(error && (error.code === "42703" || error.code === "PGRST204" || /scoring_version|normalization_version|rating_version/i.test(error.message ?? "")));
}
...
export async function POST(request: Request) {
  ...
  const { data: challenge, error: challengeError } = await admin
    ...
    .select("encoded_board,board_payload,rules_version,category_set_version,dataset_version")
    .eq("challenge_date", body.challengeDate)
    .eq("difficulty", difficulty)
    .single();
  ...
  try {
    ...
    const storedChallenge = challenge as StoredChallenge;
    const config = configForDifficultyDimensions(
      difficulty,
      round.categories.length,
      round.bank.length,
      storedChallenge.rules_version !== RULES_VERSION,
    );
    if (!config) {
    ...
    const versionedPayload = {
      ...
      board_normalization_version: BOARD_NORMALIZATION_VERSION,
      leaderboard_rating_version: LEADERBOARD_RATING_VERSION,
      rules_version: (challenge as StoredChallenge).rules_version,
