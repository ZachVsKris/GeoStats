...
export function scoreCategoryQuality(dataset: CategoryDataset, nowYear=new Date().getUTCFullYear()): CategoryQuality {
  ...
  const coverageScore=Math.round(45*clamp(coverage/floor));
...
export function runtimeQualityScore(category: Category, coverage:number, latestYear:number|null, variation=0.8){
 const floor=Math.max(20,category.coverageFloor??100); const coverageScore=45*clamp(coverage/floor); const age=latestYear===null?99:Math.max(0,new Date().getUTCFullYear()-latestYear);
