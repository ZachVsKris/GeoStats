...
async function loadCandidateDatasets(seed:string,targetCount=72):Promise<RoundCategory[]>{
 ...
 for(let offset=0;offset<shuffled.length&&loaded.length<targetCount;offset+=10){
  ...
  for(const result of results){if(result.status!=="fulfilled")continue;const d=result.value;const q=scoreCategoryQuality(d);if(!q.eligible)continue;const type=roundType(d.category);if((typeCounts.get(type)??0)>=8)continue;loaded.push(d);ids.add(d.category.id);typeCounts.set(type,(typeCounts.get(type)??0)+1);}
 }
 return loaded;
}
...
export function scoreBoard(round:Round,config:RoundConfig):ScoreBreakdown{
 ...
 const avgRank=ranks.reduce((a,b)=>a+b,0)/Math.max(1,ranks.length);const avgGap=gaps.reduce((a,b)=>a+b,0)/Math.max(1,gaps.length);const rankTarget=config.difficulty==="easy"?8:config.difficulty==="normal"?16:24;const gapTarget=config.difficulty==="easy"?.30:config.difficulty==="normal"?.15:.07;
...
export async function generateDailyTrio(countries:CountryInfo[],date:string,fixed:Partial<DailyTrio>={}):Promise<{trio:DailyTrio;diagnostics:GenerationDiagnostics;scores:Record<DailyDifficulty,ScoreBreakdown>}>{
 const seed=`DAILY-TRIO-${date}`;const available=await loadCandidateDatasets(seed,72);const required=DAILY_DIFFICULTIES.reduce((n,d)=>n+ROUND_CONFIGS[d].categoryCount,0);const diagnostics:GenerationDiagnostics={eligibleDatasets:available.length,requiredDatasets:required,attempts:0,validCandidates:{easy:0,normal:0,expert:0}};
