...
class Importer(WarehouseImporter):
 ...
 def discover(self):
  desc='UNESCO-listed living traditions and practices associated with the country, such as festivals, music, dance, rituals and crafts. These are different from monuments and World Heritage sites; multinational traditions count for every participating country.'
  r=IndicatorRule(key='most-ich-elements',title='Most living cultural traditions recognized by UNESCO',description=desc,plain_language_description=desc,technical_definition='Count of current UNESCO Intangible Cultural Heritage elements associated with the country; multinational elements count for every participating country.',unit_explanation='Current UNESCO-listed living cultural traditions associated with the country',family='Culture',icon='🎭',unit='traditions',value_type='total',ranking_direction='high',include=('ich001',),min_coverage=80,evidence_tier='A',source_priority=6,specificity_score=98,recognizability_score=94,understandability_score=98,fun_score=97)
