...
async function loadDetail(admin: SupabaseClient, id: string) {
  ...
  let similarQuery = admin
    .from("category_review_queue_v15")
    .select("id,effective_title,source_organization,source_indicator_code,effective_semantic_group,editorial_status,computed_playable_v15,duplicate_of")
    .neq("id", id)
    .limit(16);
...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (loaded.detail && "error" in loaded.detail) return NextResponse.json({ error: loaded.detail.error.message }, { status: 500 });
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data: policy, error: policyError } = await auth.admin
    .from("category_review_queue_v15")
    .select("id,editorial_status,computed_playable_v15,recommended_title,semantic_group")
    .eq("id", id)
    .single();
