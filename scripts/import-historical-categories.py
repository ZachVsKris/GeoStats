...
def observed_threshold_crossing_years(series: dict[str, dict[int, float]], threshold: float) -> dict[str, int]:
    """Find exact annual threshold crossings without inventing dates across gaps.

    A country is included only when year Y is >= threshold and Y-1 is present and
    below threshold. Countries already above the threshold at their first source
    observation are intentionally omitted (left-censored). This makes the recent
    high-end ranking reproducible without pretending the source tells us an older
    crossing date that it does not actually observe.
    """
    crossings: dict[str, int] = {}
    for iso3, values in series.items():
        for year in sorted(values):
            if values[year] < threshold:
                continue
...
class WorldBankHistoricalMilestonesImporter(WarehouseImporter):
            ...
            candidates.append(CandidateDefinition(
                ...
                metadata={
                    ...
                    "source_query": {
                        ...
                        "crossing_rule": "Y >= threshold and Y-1 < threshold; consecutive annual observations required",
                    },
                    ...
                    "derivation_version": "geostats-v16.2.4-world-bank-milestones-v1",
                    "broadDomain": "history",
    ...
    def fetch_observations(self, candidate: CandidateDefinition) -> list[SourceObservation]:
        ...
        code = str(spec["code"])
        crossings = observed_threshold_crossing_years(self._load_series(code), float(spec["threshold"]))
        rows = [SourceObservation(
            country_iso3=iso3,
            country_name=canonical_country_name(iso3),
            data_year=SNAPSHOT_YEAR,
            ...
            evidence_status="official",
            metadata={"indicator": code, "threshold": spec["threshold"], "crossing_year": year},
...
def main(argv: list[str] | None = None) -> int:
    ...
    for importer in importers:
        ...
        expected = 2 if importer.source_slug == "ipu" else (4 if importer.source_slug == "worldbankhistory" else 1)
