...
def _unit_signature(*values: Any) -> str:
    ...
    # Currency/price basis must be resolved before generic currency matching.
    # Cross-country rankings cannot silently equate local currency, current US$,
    # constant-price US$, or PPP/international-dollar measures.
    if re.search(r"current\s+(?:lcu|local currency)|local currency.*current", text):
        return "currency-current-local"
    if re.search(r"constant\s+(?:lcu|local currency)|local currency.*constant", text):
        return "currency-constant-local"
    if re.search(r"constant.*(?:us dollar|usd|us\$)|(?:us dollar|usd|us\$).*constant", text):
        return "currency-constant-usd"
    if re.search(r"current.*(?:us dollar|usd|us\$)|(?:us dollar|usd|us\$).*current", text):
        if re.search(r"per capita|per person|/person", text):
            return "currency-current-usd-per-person"
        return "currency-current-usd"
    if re.search(r"constant.*international dollar|ppp.*constant|constant.*ppp", text):
        return "currency-constant-ppp"
    if re.search(r"international dollar|ppp", text):
        return "currency-ppp"
    ...
    # Match the largest denominator first. The previous ordering allowed
    # "per 100,000" to be captured by the "per 100" branch.
    denominator = ""
    if re.search(r"live births?", text): denominator = "-live-births"
    elif re.search(r"adult(?:s)?", text): denominator = "-adults"
    elif re.search(r"workers?|employees?|employed", text): denominator = "-workers"
    ...
    if re.search(r"per 10,?000", text):
        return "per-10000" + denominator
    if re.search(r"per 1,?000", text):
        return "per-1000" + denominator
    if re.search(r"per 100(?:\s|$| people| population| adults| workers)", text):
        return "per-100" + denominator

    if re.search(r"%|percent|percentage|share of", text):
        basis = ""
        if re.search(r"gdp|gross domestic product", text): basis = "-of-gdp"
        elif re.search(r"exports?", text): basis = "-of-exports"
    ...
    if re.search(r"kg.*ha|kilogram.*hectare", text):
        return "kg-per-ha"
    if re.search(r"million\s+tonnes?|million\s+tons?", text):
        return "million-tonnes"
    if re.search(r"thousand\s+tonnes?|thousand\s+tons?", text):
...
def units_compatible(stored_unit: Any, expected_unit: Any, official_name: Any = "", official_unit: Any = "") -> bool:
    ...
    if official not in generic:
        # If the curated rule declares a concrete unit/basis, it must agree with
        # the provider metadata. This catches semantically wrong rules even when
        # the stored display unit happens to mirror the provider.
        if expected not in generic and expected != official:
            current_usd_rule_alias = expected == "usd" and official == "currency-current-usd"
            current_usd_pc_rule_alias = expected == "usd-per-person" and official == "currency-current-usd-per-person"
            if not (current_usd_rule_alias or current_usd_pc_rule_alias):
                return False
        # A plain USD display label is a harmless shorthand only for a current-price
        # US-dollar series. It must never erase a constant-price or PPP basis.
        current_usd_alias = (stored == "usd" and official == "currency-current-usd")
        current_usd_pc_alias = (stored == "usd-per-person" and official == "currency-current-usd-per-person")
        return stored == official or current_usd_alias or current_usd_pc_alias or (expected == official and stored == expected)
