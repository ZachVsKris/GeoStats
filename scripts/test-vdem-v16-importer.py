...
assert len(m.SPECS)==7
assert {x.key for x in m.SPECS}=={'electoral-democracy','liberal-democracy','freedom-expression','clean-elections','rule-law','political-corruption','civil-society-participation'}
assert next(x for x in m.SPECS if x.key=='rule-law').aliases==('v2x_rule',)
...
print('V-Dem v16 curated importer tests passed.')
