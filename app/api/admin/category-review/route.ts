...
export async function GET(request: Request) {
  ...
  const [categoriesResult, overviewResult, sourceResult] = await Promise.all([
    ...
    auth.admin.from("category_review_overview_v16").select("*").maybeSingle(),
    auth.admin
      .from("category_review_workbench_v16")
      .select("source_organization,editorial_status,hard_gate_ready,computed_playable_v16,metadata")
      .limit(10000),
  ]);
    ...
    return NextResponse.json(
      {
        error: migrationMissing
          ? "Run RUN_THIS_IN_SUPABASE_FOR_V16_0.sql before using the Category Review Workbench."
          : error.message,
