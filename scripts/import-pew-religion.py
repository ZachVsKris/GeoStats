...
def _load_rows(path: Path) -> list[dict[str, Any]]:
    if path.suffix.lower()==".csv":
        with path.open(newline="",encoding="utf-8-sig") as handle: return _parse_rows([list(r) for r in csv.reader(handle)])
    workbook=load_workbook(path,read_only=True,data_only=True)
    best=[]; errors=[]
    for sheet in workbook.worksheets:
        rows=[list(r) for r in sheet.iter_rows(values_only=True)]
        try: parsed=_parse_rows(rows)
        except RuntimeError as error: errors.append(f"{sheet.title}: {error}"); continue
        if len(parsed)>len(best): best=parsed
    if not best: raise RuntimeError("No usable country sheet found. "+"; ".join(errors[:5]))
    return best
