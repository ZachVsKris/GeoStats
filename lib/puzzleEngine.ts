...
function fixedCandidate(round: Round, difficulty: DailyDifficulty, recentCountryExposure?: Record<string, number>) {
  return candidateFromRound(round, scoreBoard(round, ROUND_CONFIGS[difficulty]).overall, recentCountryExposure);
}
...
export function generateDailyTrioFromLoadedCatalog(
  ...
  if (runJointFirst && jointReserveMs > 0) {
    ...
    const guided = constructGuidedTrio(loaded, countries, seed, fixed, profiles, guidedDeadline, diagnostics, options.recentCountryExposure);
    if (guided) {
  ...
  if (!runJointFirst && options.jointSearch !== false && Date.now() < overallDeadline) {
    const guided = constructGuidedTrio(loaded, countries, seed, fixed, profiles, overallDeadline, diagnostics, options.recentCountryExposure);
    if (guided) {
      diagnostics.generationProfile = guided.profile;
      diagnostics.preferenceWarnings = dailyTrioPreferenceWarnings(guided.trio);
      diagnostics.elapsedMs = Date.now() - startedAt;
