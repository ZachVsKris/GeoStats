...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--include-rejected", action="store_true", help="Explicitly include rejected rows only when a documented implementation/source blocker has changed.")
    parser.add_argument("--repair-evidence", default="", help="Required explanation when --include-rejected is used; recorded in category metadata.")
...
def main() -> int:
    ...
    if args.include_rejected and not str(args.repair_evidence).strip():
        raise SystemExit("--include-rejected requires --repair-evidence describing the changed blocker or methodology.")
    ...
    if not args.include_rejected:
        stored = [row for row in stored if str(row.get("review_status") or "").lower() != "rejected" and str(row.get("curation_status") or "").lower() not in {"excluded", "rejected"}]
            ...
            patch: dict[str, Any] = {
                ...
                "metadata": {
                    **metadata,
                    ...
                    "v16_2_6_repair_evidence": str(args.repair_evidence).strip() or None,
                    "v16_2_6_same_source_retry": bool(args.include_rejected),
                },
                "updated_at": now,
            }
