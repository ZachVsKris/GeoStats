...
def test_catalog_is_curated() -> None:
    ...
    coffee = next(candidate for candidate in candidates if candidate.rule.key == "most-coffee-exported")
    assert coffee.metadata["source_query"]["reporterCode"] == ""
    assert coffee.metadata["source_query"]["reporterSelection"] == "all reporters"
    assert coffee.metadata["source_query"]["partnerCode"] == "0"
    assert coffee.metadata["source_query"]["partner"] == "World"
