...
def classify_semantics(source_slug: str, candidate: CandidateDefinition, concept_group: str) -> SemanticProfile:
    ...
    if source_slug in {"unmembership", "constitute", "ipu"}:
        return SemanticProfile("historical-state-institutions", topic)
    if source_slug == "unwpp":
        return SemanticProfile(str(candidate.metadata.get("strategyFamily") or "demographics"), topic)
    if source_slug == "worldbankclimate":
        return SemanticProfile(str(candidate.metadata.get("strategyFamily") or "climate"), topic)
    if source_slug == "imfweo":
        return SemanticProfile(str(candidate.metadata.get("strategyFamily") or "macro-economy"), topic)
    if source_slug == "unescoich":
        return SemanticProfile("cultural-heritage", topic)
    if source_slug == "noaatsunami":
        return SemanticProfile("tsunami-history", topic)
    if source_slug == "whoghed":
        return SemanticProfile("health-spending", topic)
    if source_slug == "undesamigrant":
        return SemanticProfile("migration-stock", topic)
    if source_slug == "wtoservices":
        return SemanticProfile("services-trade", topic)
    if source_slug == "untourismdirect":
        return SemanticProfile("tourism", topic)
