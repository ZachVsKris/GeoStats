...
NESTED_FINDEX = re.compile(
    r"account ownership.*financial institution.*mobile[- ]money.*(?:young adults|ages? 15[-–]24)",
    re.I,
)
...
def _copy_is_playable(name: str, title: str, description: str) -> bool:
    clean = re.sub(r"\s+", " ", title).strip()
    words = re.findall(r"[A-Za-z0-9]+(?:[-’'][A-Za-z0-9]+)*", clean)
    commas = clean.count(",")
    generic = not description or re.match(
        r"^(?:compare countries using|compare the official country value|official country value for this measure)",
        description,
        re.I,
    ) is not None
    if NESTED_FINDEX.search(clean) or NESTED_FINDEX.search(name):
        return False
    if len(clean) > 96 or len(words) > 16:
        return False
    if len(clean) > 82 and commas >= 2:
        return False
    if generic and len(clean) > 68:
        return False
    return True
