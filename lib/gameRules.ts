...
export const LEGACY_V16_2_3_ROUND_CONFIGS: Record<DailyDifficulty, RoundConfig> = {
  easy: {
    difficulty: "easy", label: "Scout", path: "/daily", randomPath: "/random/easy",
    categoryCount: 4, countryCount: 5, decoyCount: 1, maxScore: 400, topFinishRank: 2,
    minRoundTypes: 3, maxSameSource: 2, maxAgricultureCategories: 1, maxFaostatCategories: 1,
    maxBroadDomain: 2, maxCountriesPerContinent: 2, pointsByRank: [100, 75, 50, 25, 0],
  },
  normal: {
    difficulty: "normal", label: "Adventurer", path: "/daily/adventurer", randomPath: "/random",
    categoryCount: 6, countryCount: 8, decoyCount: 2, maxScore: 600, topFinishRank: 3,
    minRoundTypes: 4, maxSameSource: 2, maxAgricultureCategories: 2, maxFaostatCategories: 2,
    maxBroadDomain: 2, maxCountriesPerContinent: 3, pointsByRank: [100, 85, 70, 55, 40, 25, 10, 0],
  },
  expert: {
    difficulty: "expert", label: "Expert", path: "/daily/expert", randomPath: "/random/expert",
    categoryCount: 8, countryCount: 10, decoyCount: 2, maxScore: 800, topFinishRank: 5,
    minRoundTypes: 5, maxSameSource: 3, maxAgricultureCategories: 2, maxFaostatCategories: 2,
    maxBroadDomain: 2, maxCountriesPerContinent: 3, pointsByRank: [100, 90, 80, 70, 60, 50, 40, 30, 20, 10],
  },
};
...
const ONE_PER_BOARD_KNOWLEDGE_CLUSTERS = new Set([
  "forced-displacement",
...
function onePerBoardCluster(category: Category) {
  const cluster = knowledgeCluster(category);
  if (ONE_PER_BOARD_KNOWLEDGE_CLUSTERS.has(cluster)) return cluster;
  if (category.productSpecificTrade || cluster === "product-exports") return "product-exports";
  return null;
}

function hasBoardClusterConflict(selected: Category[], category: Category) {
  const cluster = onePerBoardCluster(category);
  return Boolean(cluster && selected.some((item) => onePerBoardCluster(item) === cluster));
}
...
export function canAddCategory(selected: Category[], category: Category, config: RoundConfig = ROUND_CONFIGS.normal) {
  ...
  if (hasBoardClusterConflict(selected, category)) return false;
  const domain = broadDomain(category);
...
export function roundHasRequiredDiversity(categories: Category[], config: RoundConfig = ROUND_CONFIGS.normal) {
  ...
  const constrainedClusters = categories.map(onePerBoardCluster).filter((cluster): cluster is string => Boolean(cluster));
  if (new Set(constrainedClusters).size !== constrainedClusters.length) return false;
...
export function strongestGlobalWinnerRankForCategory(category: Category, coverage: number) {
  // Chronology questions are about relative dates, not whether their winner is
  // a conventional "top 30" statistical country. They still must pass the
  // database coverage/ranking-completeness and all other integrity gates.
  if (measureKind(category) === "historical") return Math.max(1, Math.floor(coverage));
  return strongestGlobalWinnerRank(coverage);
}

export function configForDimensions(categoryCount: number, countryCount: number): RoundConfig | null {
  return Object.values(ROUND_CONFIGS).find((config) => config.categoryCount === categoryCount && config.countryCount === countryCount)
    ?? Object.values(LEGACY_V16_2_3_ROUND_CONFIGS).find((config) => config.categoryCount === categoryCount && config.countryCount === countryCount)
    ?? null;
}

export function configForDifficultyDimensions(
  difficulty: DailyDifficulty,
  categoryCount: number,
  countryCount: number,
  allowLegacy = false,
): RoundConfig | null {
  const current = ROUND_CONFIGS[difficulty];
  if (current.categoryCount === categoryCount && current.countryCount === countryCount) return current;
  if (!allowLegacy) return null;
  const legacy = LEGACY_V16_2_3_ROUND_CONFIGS[difficulty];
  return legacy.categoryCount === categoryCount && legacy.countryCount === countryCount ? legacy : null;
}
...
export function pointsForBankSize(countryCount: number) {
  const config = Object.values(ROUND_CONFIGS).find((item) => item.countryCount === countryCount)
    ?? Object.values(LEGACY_V16_2_3_ROUND_CONFIGS).find((item) => item.countryCount === countryCount);
