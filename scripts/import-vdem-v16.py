...
SOURCE_ORG='V-Dem Institute';SOURCE_DATASET='V-Dem Country-Year Full+Others v16';SOURCE_PAGE='https://www.v-dem.net/data/the-v-dem-dataset/';CODEBOOK='https://v-dem.net/documents/55/v-dem_codebook_v16.pdf'
# Curated rather than exhaustive: keep the most legible, meaningfully distinct
# government/civics concepts and avoid proliferating overlapping democracy
# subindices. V-Dem v16 was published March 2026 and ends in calendar year 2025.
SPECS=(
...
class Importer(WarehouseImporter):
 ...
 def _data(self):
  if self._parsed is None:
   if not self.input_path:raise RuntimeError('V-Dem v16 importer requires the exact official country-year release via --input.')
 ...
 def discover(self):
  ...
  for s in SPECS:
   ...
   desc=f'{s.title} in the V-Dem Country-Year v16 dataset for 2025.'
...
def main():
 ...
 result=Importer(None if a.dry_run else SupabaseWarehouse(u,k),a.input,a.dry_run).run(only_keys=set(a.only) or None)
 if not a.dry_run and (result['categories_processed'] != len(SPECS) or result['observations_inserted'] <= 0 or result['source_integrity_failed']):
  raise SystemExit(f'V-Dem v16 import failed closed: {result}')
 print(result)
