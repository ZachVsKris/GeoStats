...
```bash
npm run check-v16-2-3
```

## Current database release

GeoStats v16.2.3 is a cumulative upgrade from a verified v16.2.1 database. Run the release in this order:

1. Take a Supabase snapshot, then run `RUN_THIS_IN_SUPABASE_FOR_V16_2_3.sql`
2. Push the v16.2.3 repository and require **Verify GeoStats v16.2.3** to pass
3. Run **Import v16.2.3 historical categories and finalize**
4. Review/download the source and category audit artifacts
5. Run `VERIFY_V16_2_3.sql` and confirm every final check is `PASS`
6. Redeploy/promote v16.2.3 after finalization so the first production Daily/Random cache snapshots use the final catalog

If Vercel auto-deploys when you push step 2, that preview/initial deployment is fine; trigger one final redeploy after steps 3–5.

Do not rerun older installers during the upgrade. See `V16_2_3_INSTALLATION.md`, `RELEASE_NOTES_V16_2_3.md`, and `VALIDATION_V16_2_3.md` for details.
