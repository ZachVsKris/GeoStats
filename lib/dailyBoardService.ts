...
import { DAILY_DIFFICULTIES, semanticFamily, type DailyDifficulty } from "./gameRules";
import { worldKnowledgeBucket, type CategoryExposure } from "./categoryGeneration";
...
export function recentCategoryExposureFromRows(rows: StoredDailyRow[], maxDays = 21): CategoryExposure {
  const dates = [...new Set(rows.map((row) => row.challenge_date))].sort().reverse().slice(0, maxDays);
  const dateWeights = new Map(dates.map((value, index) => {
    const day = index + 1;
    const weight = day <= 3 ? 18 : day <= 7 ? 10 : day <= 14 ? 4 : 1.25;
    return [value, weight] as const;
  }));
  const exposure: CategoryExposure = { category: {}, family: {}, bucket: {} };
  for (const row of rows) {
    const weight = dateWeights.get(row.challenge_date);
    if (!weight || !row.board_payload) continue;
    try {
      ...
      // Category repetition is a soft preference. Malformed legacy rows never block Daily generation.
    }
  }
  return exposure;
}

async function loadRecentCategoryExposure(admin: any, beforeDate: string) {
  const { data, error } = await admin
    .from("daily_challenges")
    .select("challenge_date,difficulty,seed,encoded_board,board_payload,rules_version,category_set_version,dataset_version")
    ...
    .limit(189);
  if (error) return { category: {}, family: {}, bucket: {} } as CategoryExposure;
  return recentCategoryExposureFromRows((data ?? []) as StoredDailyRow[]);
}
...
async function publishAtomically(admin: any, date: string, rows: ReturnType<typeof roundRow>[]) {
  ...
  if (error) {
    ...
    if (/could not find the function|publish_daily_trio_v16.*does not exist|schema cache.*publish_daily_trio_v16/i.test(message)) {
      throw new Error("The Daily publication RPC is unavailable. Install RUN_THIS_IN_SUPABASE_FOR_V16_2_5.sql and reload the Supabase schema cache.");
    }
