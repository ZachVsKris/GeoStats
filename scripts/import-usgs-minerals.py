...
class Importer(WarehouseImporter):
 ...
 def discover(self):
  ...
  for key in keys:
   ...
   out.append(CandidateDefinition(rule,f'USGS-MCS:{key}',title,SOURCE_PAGE,{'source_page_url':SOURCE_PAGE,'methodology_url':SOURCE_PAGE,'broadDomain':'resources','knowledgeCluster':'mineral-production','strategyFamily':f'mineral-production:{key}','boardDescription':desc,'source_file_sha256':self.source_sha256}))
  return out
 def fetch_observations(self,c):
  return [SourceObservation(i,canonical_country_name(i,i),y,v,SOURCE_PAGE,f'{c.rule.key}:{i}:{y}','official') for key,i,y,v,u in self.rows if key==c.rule.key]
