...
function assertTop20(round) {
  ...
  for (const dataset of round.categories) {
    const ranks = dataset.ranked.filter((row) => countryIds.has(row.countryId)).map((row) => row.globalRank);
    const best = ranks.length ? Math.min(...ranks) : Infinity;
    if (best > 20) throw new Error(`${dataset.category.id} violates universal Top-20 winner rule with best displayed rank ${best}.`);
  }
}
...
for (const difficulty of ['easy', 'normal', 'expert']) assertTop20(first.trio[difficulty]);
...
if (maxSeededExposure > averageSeededExposure * 3.2) {
...
// Anchor selection is the reachability/exposure layer: across a large deterministic
// sample, every synthetic playable category must actually receive anchor chances.
const anchorExposure = new Map();
for (let index = 0; index < 1200; index += 1) {
  const id = selectSeededAnchorCategoryId(`ATLAS-ANCHOR-${index}`, 'normal', loaded);
  if (!id) throw new Error('Seeded anchor selector returned no category.');
  anchorExposure.set(id, (anchorExposure.get(id) || 0) + 1);
}
const missingAnchors = datasets.filter((dataset) => !anchorExposure.has(dataset.category.id));
if (missingAnchors.length) throw new Error(`Playable categories never selected as anchors: ${missingAnchors.map((item) => item.category.id).join(', ')}`);
