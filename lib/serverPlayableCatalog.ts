...
async function loadRows() {
  ...
  if (result.error) {
    ...
    if (/schema cache/i.test(message)) {
      throw new Error("Supabase has not refreshed its REST schema cache for the v16.2.5 catalog. Run NOTIFY pgrst, 'reload schema'; and retry after 30 seconds.");
    }
    if (/category_runtime_review_v16_2|does not exist|relation .* not found/i.test(message)) {
      throw new Error("The GeoStats v16.2.5 catalog migration is not installed. Run RUN_THIS_IN_SUPABASE_FOR_V16_2_5.sql.");
    }
...
const loadCachedRows = unstable_cache(
  loadRows,
  ["geostats-category-catalog-rows-v16.2.5"],
