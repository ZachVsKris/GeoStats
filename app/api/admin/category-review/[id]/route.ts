...
async function loadDetail(admin: SupabaseClient, id: string) {
  const categoryResult = await admin
    .from("category_review_workbench_v16")
    .select("*")
    .eq("id", id)
    .maybeSingle();
  ...
  let similarQuery = admin
    .from("category_review_workbench_v16")
    .select("id,effective_title,source_organization,source_indicator_code,effective_semantic_group,editorial_status,computed_playable_v16,duplicate_of")
    .neq("id", id)
    .limit(16);
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!previous) return NextResponse.json({ error: "Category review state not found. Run the v16.1 SQL installer." }, { status: 409 });
  ...
  const { error: refreshError } = await auth.admin.rpc("refresh_v16_runtime_catalog");
  ...
  const { data: policy, error: policyError } = await auth.admin
    .from("category_review_workbench_v16")
    .select("id,editorial_status,computed_playable_v16,recommended_title,semantic_group")
    .eq("id", id)
    .single();
  ...
  const categoryUpdate: Record<string, unknown> = {
    ...
    curation_reason: `GeoStats v16.1 authoritative category review state: ${policy.editorial_status}.`,
    curation_version: "geostats-v16.1-review-v1",
    content_review_status: legacyEditorialStatus,
    content_review_reason: `GeoStats v16.1 authoritative category review state: ${policy.editorial_status}.`,
    content_review_version: "geostats-v16.1-review-v1",
    player_quality_status: policy.editorial_status === "approved"
      ? "approved"
      : ["rejected", "duplicate"].includes(policy.editorial_status)
        ? "blocked"
        : "caution",
    player_quality_reason: `GeoStats v16.1 authoritative category review state: ${policy.editorial_status}.`,
    enabled: Boolean(policy.computed_playable_v16),
    eligible_daily: Boolean(policy.computed_playable_v16),
  };
  ...
  if (policy.computed_playable_v16 && typeof policy.recommended_title === "string" && policy.recommended_title.trim()) {
