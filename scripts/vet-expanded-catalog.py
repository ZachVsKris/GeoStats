...
EXPANSION_SOURCES = {
    ...
    "FAOSTAT Food Balances", "World Bank",
}
...
def main() -> int:
    ...
    if args.source == "expansion":
        target_rows = [
            ...
            if row.get("source_organization") in EXPANSION_SOURCES
            and (
                row.get("source_organization") != "World Bank"
                or (isinstance(row.get("metadata"), dict) and row["metadata"].get("manual_review_required"))
            )
        ]
    ...
    for row in target_rows:
        ...
        results.append({
            ...
            "tie_share": tie_share,
            "vetting_version": "geostats-v15.9-auto-vetting-v1",
        })

    if results:
        warehouse._request(
            "POST",
            "category_auto_vetting_v15_9?on_conflict=category_id",
            results,
