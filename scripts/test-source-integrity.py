...
assert not units_compatible("per 100 people", "per 100,000 people", "Road deaths per 100,000 people")
assert not units_compatible("per 100,000 adults", "per 100,000 live births", "Maternal mortality per 100,000 live births")
assert not units_compatible("% of GDP", "% of exports", "Exports of goods and services (% of GDP)")
...
comtrade_rule = IndicatorRule(
    key="test-comtrade", title="Largest test exports", description="Test exports.", family="Trade", icon="📦",
    unit="USD", value_type="total", ranking_direction="high", include=("09",), min_coverage=3,
)
def comtrade_candidate(partner_value: str) -> CandidateDefinition:
    return CandidateDefinition(
        rule=comtrade_rule, source_indicator_code="09", source_indicator_name="HS 09 exports to World",
        source_url="https://comtradeplus.un.org/",
        metadata={"source_query": {"cmdCode": ["09"], "flowCode": "X", "partnerCode": partner_value}},
    )
assert source_identity_checks("comtrade", comtrade_candidate("0"))["world_partner_selected"] is True
assert source_identity_checks("comtrade", comtrade_candidate("0 (World)"))["world_partner_selected"] is True
assert source_identity_checks("comtrade", comtrade_candidate("World"))["world_partner_selected"] is True
assert source_identity_checks("comtrade", comtrade_candidate("1"))["world_partner_selected"] is False
...
print("GeoStats v16.2.4 source-integrity fixture tests passed.")
