...
export default function GeoSecondComingGame({ initialDifficulty = DEFAULT_DIFFICULTY, mode = "daily" }: GeoSecondComingGameProps = {}) {
  ...
  async function loadDailyRound(nextDifficulty: DailyDifficulty, existingCountries: CountryInfo[] = []) {
    ...
    try {
      ...
      if (cached?.fallback) clearCachedDaily(date);
      const usableCached = cached?.fallback ? null : cached;
      const cachedPacked = usableCached?.[nextDifficulty];
      if (cachedPacked) {
        try {
          ...
          setFallbackPractice(Boolean(usableCached?.fallback));
          setBoardNotice(usableCached?.warning ?? "");
          setRound(restored);
          setStatus("");
      ...
      if (saved.fallback) clearCachedDaily(date);
      else writeCachedDaily(date, saved);
