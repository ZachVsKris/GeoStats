...
async function loadRows() {
  ...
  const result = await admin
    .from("category_runtime_review_v16")
    .select(V16_SELECT)
    .order("quality_score", { ascending: false })
    .limit(5000);

  if (result.error) {
    const missingMigration = /category_runtime_review_v16|does not exist|schema cache/i.test(result.error.message);
    throw new Error(
      missingMigration
        ? "The v16 category catalog is not installed. Run the current Supabase installer."
...
const loadCachedRows = unstable_cache(
  loadRows,
  ["geostats-category-catalog-rows-v16.0"],
